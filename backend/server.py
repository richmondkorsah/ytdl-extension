...
def stream_from_worker_result(data, video_title, channel_name, resolution, codec, format_str):
    ...
    if len(stream_urls) == 1:
        # Single pre-muxed or audio-only stream — proxy directly from CDN
        ...
        # Separate video + audio streams — merge on-the-fly with FFmpeg
        # FFmpeg reads directly from CDN URLs; output is fragmented MP4 piped to stdout
        cmd = [
            "ffmpeg",
            "-i", stream_urls[0],
            "-i", stream_urls[1],
...
if __name__ == "__main__":
    ...
    print(f"Mode: {'RENDER (worker relay)' if RENDER_MODE else 'LOCAL'}")
    ...
    if RENDER_MODE:
        print("  GET /worker/jobs - Worker polls for pending jobs")
        print("  POST /worker/result - Worker posts resolved results")
    print("="*60 + "\n")
